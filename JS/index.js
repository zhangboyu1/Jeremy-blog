...
class Scroll {
    handleOnScroll = () => {
        ...
        if (scrollPosition > 18) {
            document.getElementById(`container`).childNodes[1].className = `header sticker-out`
        } if (!scrollPosition) {
            document.getElementById(`container`).childNodes[1].className = `header sticker-in`
        }
    }
...
function myFunction() {
    ...
    if (x.className === "headerElement header_navBar") {
        x.className += " responsive";
        y[0].style.marginTop = '150px'
    } else {
